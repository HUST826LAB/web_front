...
*0.设置用户绘制线条颜色信息canvasData.draw.rgba[r,g,b,a],设置高分参照颜色canvasData.original.rgba[r,g,b,a],设置低分参照颜色canvasData.original.rgba[r,g,b,a]
...
export default {
  calMatching: function(c) {
    ...
    for (var i = 0; i < canvasData.original.data.length; i++) {
      ...
      if (canvasData.original.data[i] == 2 && canvasData.draw.data[i] == 1) {
        sumMatching1++;
      }
    ...
    canvasData.matching = (sumMatching * 1 + sumMatching1 * 0.5) / sumDraw;
  ...
  getcanvasData: function(c) {
    ...
    var originMax = canvasData.original.rgba;
    var originMin = canvasData.original.rgba1;
    if (canvasData.state) { //玩家绘制之前采集原始像素信息
      canvasData.original.data = [];
      for (var i = 0; i < imgData.length; i += 4) {
        if (originMax[0] == imgData[i + 0] //像素信息与原始线条颜色相同，
          &&
          originMax[1] == imgData[i + 1] &&
          originMax[2] == imgData[i + 2]) {
          canvasData.original.data[(i - i % 4) / 4] = 1;
        } else if (originMin[0] == imgData[i + 0] //像素信息与原始线条颜色相同，
          &&
          originMin[1] == imgData[i + 1] &&
          originMin[2] == imgData[i + 2]) {
          canvasData.original.data[(i - i % 4) / 4] = 2;
        } else {
